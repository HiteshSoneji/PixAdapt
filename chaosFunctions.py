...
def rosslerMap(height, width, a, b, c, x0):
    ...
    rossler_map = (rossler_map*255).astype('uint8')
    x = []
    y = []
    z = []
...
def henonMap(height, width, x, y, a, b):
    ...
    for i in range(1, size):
        x0, y0 = 1 - (a*x**2) +y0, b*x0
        xs[i], ys[i] = x0, y0
    return np.array([np.binary_repr(i, width = 8) for i in (xs*255).astype('uint8')]).reshape((height, width))
