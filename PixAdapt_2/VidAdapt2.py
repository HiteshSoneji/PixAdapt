...
def run_sim(img, genotype1, seq):
    ...
    cost, seq = encryptImage(img, genotype1, seed_enc)
    costs.append(cost)
    ...
    while True:
        best = 33.46
        if abs(best - cost) < 0.4: #chaning to 0.1 from 1
            break
        genotype2 = cp.deepcopy(genotype1)
        mutate_genotype(genotype2)
        cost2, seq = encryptImage(img, genotype2, seed_enc)
        costs.append(cost2)
...
def encryptImage(img, genotype, seq):
    ...
    return calc_UACI(chaos_encrypted_image, img), chaos_encrypted_image
...
log_map_seed = create_continous_genes(0.1, 0.99) # chaning from 0.01 to 1
...
for img_num, img in enumerate(frames):
    ...
    genotype1, seq = run_sim(img, genotype1, seq)
    print(f"METRICS: ENTROPY = {round(shannon_entropy(seq.astype('uint8')), 4)} | UACI = {round(calc_UACI(seq, img),4)} | NPCR = {round(calc_NPCR(seq, img), 4)}\n\n")
    plt.imshow(seq)
    plt.show()